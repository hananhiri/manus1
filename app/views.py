...
def index():
    ...
    return render_template("index.html", data=data)  # hapus iki

    # users count
    #users_count = dbentity.users.find().count()                    #-------------------------------------------- KENDALA
    #last_user = [i for i in dbentity.users.find()][-1]

    #return render_template("index.html", data=data, users_count=users_count, 
    #                       last_user=last_user
    #)
    # End OF TAMBAHAN 8 JAN 15
    ...
    if request.method == "POST":
        ...
        for term in terms:
            ...
            if not pdfterms.term.find_one({"term": term}):
                pdfterms.term.insert({"term": term})
            # update terms document status dari 0 => 1
            terms.term.update({"term": term}, {"$set": {"status": 1}})     #-----------------------------------------------------------------------Ingak Ingak
...
def grabber():
    ...
    if request.method == "POST":
        ...
        if data is not None:
            ...
            # built url
            # url = "https://www.google.com/search?q=%s+filetype:pdf&num=10" % keyword.replace(" ", "+")    #----------Source pdf google
...
def stats():
    ...
    update db terms: {$inc: {hits: 1}}    ------------------------------------------------------- Ingak Ingak
    """
